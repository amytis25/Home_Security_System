...
ENSC 351 Fall 2025 
Assignment 3
NOTE: Added to the code to fix volume and tempo button initialization issue, 
and issues with polling. */
...
// Initialize to null so the UI requests and uses the device values on load
var volume = null;
var tempo = null;
...
$(document).ready(function() {
	...
	// Setup a repeating function (every 1s) for uptime only. Removing
	// periodic volume/mode/tempo polling because they send undefined/null
	// arguments which propagate as literal "null" to the BeatBox UDP server.
	window.setInterval(function() { sendCommandToServer('read-uptime'); }, 1000);

	// Request current device state so the UI fields initialize correctly
	// (prevents sudden jumps when user first adjusts controls).
	// Use short delays so socket handlers are fully established.
	setTimeout(function() { sendCommandToServer('read-volume'); }, 100);
	setTimeout(function() { sendCommandToServer('read-tempo'); }, 150);
